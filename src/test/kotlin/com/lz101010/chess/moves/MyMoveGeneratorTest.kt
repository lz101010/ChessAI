...
class MyMoveGeneratorTest {

    @OptIn(ExperimentalTime::class)
    //@Test
    fun benchmark() {
        val board = BoardGenerator.fromFen("3b4/8/1B6/KQ4qk/R7/8/8/r7 w - - 0 1")

        val mine = measureTime {
            for (i in 0 .. 100_000) {
                MyMoveGenerator.find(board)
            }
        }
        val theirs = measureTime {
            for (i in 0 .. 100_000) {
                MoveGenerator.find(board)
            }
        }
        println(mine.inWholeMilliseconds)
        println(theirs.inWholeMilliseconds)
    }

    @Test
    fun findDefaultBoardMoves_passes() {
        assertThat(MyMoveGenerator.find(Board.default))
            .hasSize(20)
    ...
    fun findBoardAfterE4Moves_passes() {
        ...
        assertThat(MyMoveGenerator.find(boardAfterE4))
            .hasSize(20)
    ...
    fun findBoardAfterE4B5Moves_passes() {
        ...
        assertThat(MyMoveGenerator.find(board))
            .hasSize(29)
    ...
    @Test
    fun findMovesForPinnedPieces_passes() {
        val board = BoardGenerator.fromFen("3b4/8/1B6/KQ4qk/R7/8/8/r7 w - - 0 1")

        assertThat(MyMoveGenerator.find(board))
            .hasSize(12)
            .contains(Move(Square.A5, Square.A6)).contains(Move(Square.A5, Square.B4))
            .contains(Move(Square.B6, Square.C7)).contains(Move(Square.B6, Square.D8))
            .contains(Move(Square.A4, Square.A3)).contains(Move(Square.A4, Square.A2))
            .contains(Move(Square.A4, Square.A1))
            .contains(Move(Square.B5, Square.C5)).contains(Move(Square.B5, Square.D5))
            .contains(Move(Square.B5, Square.E5)).contains(Move(Square.B5, Square.F5))
            .contains(Move(Square.B5, Square.G5))
    }

    @Test
    fun findKingMoves_passes() {
        val board = BoardGenerator.fromFen("8/8/2K1k3/8/8/8/8/8 w - - 0 1")

        assertThat(MyMoveGenerator.find(board))
            .hasSize(5)
            .contains(Move(Square.C6, Square.C7)).contains(Move(Square.C6, Square.B7))
            .contains(Move(Square.C6, Square.B6)).contains(Move(Square.C6, Square.B5))
            .contains(Move(Square.C6, Square.C5))
    }
